...
除了`imread`和`imreadmulti`，OpenCV 还支持使用`imdecode`函数从存储缓冲区读取图像。 如果图像未存储在磁盘上或需要从网络读取数据流中，则此功能特别有用。 用法与`imread`函数几乎相同，除了需要为其提供数据缓冲区而不是文件名。
