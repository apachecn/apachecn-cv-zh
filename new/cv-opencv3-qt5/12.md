...
在本节中，我们将学习 Qt Quick 应用程序项目的结构。 与 Qt Widgets 应用程序项目类似，使用 Qt Creator 创建新项目时，会自动创建 Qt Quick 应用项目所需的大多数文件，因此您实际上并不需要记住所有的最低要求，但是仍然重要的是要理解如何处理 Qt Quick 应用程序的一些基本概念，以便能够进一步扩展它，或者，如我们将在本章后面的部分中了解的那样，在 QML 文件中集成和使用 C++ 代码。
...
该`main.cpp`与创建 Qt Widgets 应用程序时所看到的完全不同。 记住，在 Qt Widgets 应用程序的`main.cpp`内部和主函数中，创建了`QApplication`，然后显示主窗口，程序进入事件循环，以便该窗口保持活动状态，并且所有事件已处理，如下所示：
...
因此，首先，请牢记我们刚才提到的内容，可以说（实际上是相对于理想情况而言），要能够在 Android 和 iOS 上构建和运行使用 Qt 和 OpenCV 编写的应用程序，我们需要确保以下东西：
