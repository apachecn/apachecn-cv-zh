...
您可以将完全相同的参数传递给`dilate`函数。 在前面的代码中，假设使用 Combo Box 小部件获取结构元素的形状，该小部件可以是`MORPH_RECT`，`MORPH_CROSS`或`MORPH_ELLIPSE`。 同样，通过使用**旋转框**小部件设置迭代计数，该小部件可以是大于零的数字。
