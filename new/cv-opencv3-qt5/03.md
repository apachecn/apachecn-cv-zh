...
如您所见，我们的自动生成的类头文件在其私有部分中已经具有`Q_OBJECT`宏。 因此，这基本上是创建作为`QObject`（或任何其他 Qt 对象）的子类的类（不仅是窗口类，而且一般是任何 Qt 类）的标准方法，该类将支持 Qt 支持的功能，例如信号和 插槽。
