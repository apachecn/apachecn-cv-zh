...
因此，我们的插件界面包含一个名为`description()`的函数，该函数旨在返回任何插件的描述以及有关该插件的有用信息。 它还包含一个名为`processImage`的函数，该函数将`OpenCV Mat`类作为输入，并返回一个作为输出。 显然，在此函数中，我们希望每个插件执行某种图像处理，过滤等操作，然后将结果提供给我们。
