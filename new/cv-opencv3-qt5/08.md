...
让我们首先在 Qt Creator 中创建一个名为`MultithreadedCV`的示例 Qt Widgets 应用程序。 以我们在本书开始章节中学到的相同方式将 OpenCV 框架添加到该项目中：在`MultithreadedCV.pro`文件中包含以下代码（请参见[第 2 章](../Text/02.html)和*） 第一个 Qt 和 OpenCV 项目*或[第 3 章](../Text/03.html)和*创建一个综合的 Qt + OpenCV 项目*，以了解更多信息）：
...
在[第 9 章](../Text/09.html)和*视频分析*中，我们会将您在本章中学到的内容与之前的各章结合起来，并以此为基础，深入研究视频处理主题。 您将了解如何从摄像机或文件中跟踪视频中的运动对象，检测视频中的运动以及更多主题，所有这些都需要处理连续的帧并保留从先前帧中计算出的内容。 换句话说，计算不仅取决于图像，而且还取决于该图像（及时）。 因此，我们将使用线程，并使用您在本章中学习的任何方法来实现您将在下一章中学习的计算机视觉算法。