...
这些特征被认为是良好的特征，要衡量特征的优劣，我们应考虑其健壮性和不变性（尤其是缩放和旋转不变性；例如，我们的面部特征（例如眼睛）不变） 脸部比例；无论脸是大还是小，您都可以轻松检测到眼睛在哪里）。 通常，为了实现这种鲁棒性，我们将检测到的特征的质量属性与用于描述特征的方法的质量属性结合起来考虑。
...
另一个要寻找的候选人是山的边缘。 我们已经在[第 3 章](3.html#aid-MSDG2 "Chapter 3. App 2 - Software Scanner")和*应用 2-软件扫描程序*中学习了如何检测边缘，因此可以轻松找到这种类型的功能。 但是，问题在于如何唯一地描述它们，因为如果查看矩形 1.1 和 1.2，您很容易将它们混淆为同一条边。 这个问题被称为孔径问题，同样，将很难匹配。
...
3.  Call the `detect` method using the following command:
...
4.  To display the detected interest points, we call `Feature2d.drawKeypoints()`:
...
3.  Select `Makefile project`, choose **Other Toolchain**, and click on **Finish**:
...
7.  Go to the **Behaviour** tab, and in the **Workbench Build Behavior** group, check **Build on resource save** and clear the **Make build target** textbox. Clear **Make build target** textbox for the **Build (Incremental build)** checkbox:
...
9.  In this folder, we will have three files:
...
13.  生成项目。
14.  We need to include some directories so that we can write C++ code and use the **Standard Template Library** (**STL**) and OpenCV. To do this, right-click on the project name | **Properties** | **C/C++ General** | **Paths and Symbols**.
...
2.  Select **SDK location**. In **Android NDK location**, select the directory where NDK is located. Note that we will build the project using the experimental Gradle plugin version 2.5; hence we need NDK version r10e:
...
7.  Now, navigate to `<OpenCV4AndroidSDKFolder>\sdk\native\libs\` and copy all the folders to your newly created `jniLibs` folder. Your project tree should look as follows:
...
6.  We loop on the normalized corners and draw a circle for the detected corner in case its response is greater than the threshold:
...
1.  Open the activity class and add the following declaration:
...
2.  Add the following method to the `Pano.cpp` file:
...
4.  Finally, open `PanoActivity` and edit `onOptionsItemSelected()` to include the following case:
...
1.  Open the `PanoActivity` class and add the following declaration:
...
2.  In `Pano.cpp`, add the following method implementation:
...
8.  Finally, we select the best matching points and draw them:
...
3.  In the activity class, edit `onOptionsItemSelected()` to include the following case:
