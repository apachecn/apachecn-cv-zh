...
3.  运行 **SDK Manager** 。
4.  对于每个已安装的 Android`X.X`，为模拟器选择一个系统映像，例如 **ARM EABI V7a System Image**：
...
*   **Java 和本机调用**：Android 应用程序是用 Java 编写的，一旦源代码被编译，便会转换为字节码，以便 Android OS 在 **Dalvik** 或 **Android Runtime**（**ART**）下运行虚拟机。
