...
初始化`cv2.BOWKMeansTrainer`时，必须指定簇数-在我们的示例中为 40。在初始化`cv2.BOWImgDescriptorExtractor`时，必须指定描述符提取器和描述符匹配器-在我们的示例中为我们之前创建的`cv2.SIFT`和`cv2.FlannBasedMatcher`对象。
