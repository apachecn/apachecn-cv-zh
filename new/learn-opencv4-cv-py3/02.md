...
为了面向对象和适应性强，我们将此功能抽象为具有`createWindow`，`destroyWindow`，`show`和`processEvents`方法的`WindowManager`类。 作为属性，`WindowManager`具有一个称为`keypressCallback`的功能对象，如果有任何按键，则会从`processEvents`调用该函数对象（如果不是`None`）。 `keypressCallback`对象必须是带有单个参数的函数，尤其是 ASCII 键代码。
