...
在`scipy.interp1d`函数的基础上，我们实现了一系列曲线过滤器，这些过滤器高效（由于使用查找数组）并且易于扩展（由于面向对象的设计）。 我们的工作包括专用曲线过滤器，可以使数字图像看起来更像胶卷照。 这些过滤器可以很容易地集成到诸如`Cameo`之类的应用程序中，如第 3 章，“用 OpenCV 处理图像”中使用我们的 Portra 胶片仿真过滤器所示。