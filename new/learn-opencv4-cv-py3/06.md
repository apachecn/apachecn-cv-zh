...
对 DoG 的快速介绍是有序的。 之前，在第 3 章和*中使用 OpenCV* 处理图像，我们讨论了低通滤波器和模糊操作，特别是`cv2.GaussianBlur()`功能。 DoG 是对同一图像应用不同的高斯滤波器的结果。 以前，我们将这种类型的技术应用于边缘检测，这里的想法是相同的。 DoG 操作的最终结果包含兴趣区域（关键点），然后将通过 SIFT 描述这些区域。
