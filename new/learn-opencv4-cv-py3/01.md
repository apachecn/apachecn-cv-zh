...
出于本书的目的，可以将 OpenNI 2 视为可选的。 它在[第 4 章](04.html)，*深度估计和分段*中使用，但在其他各章或附录中未使用。
...
如果您打算分发依赖于 OpenCV 的非免费内容的软件，则应自己调查专利和许可问题如何在特定国家和特定用例中应用。 OpenCV 的非免费内容包括获得专利的 SIFT 和 SURF 算法的实现，我们将在[第 6 章](06.html)，*中介绍图像检索和使用图像描述符搜索*的实现。
...
要获取 OpenNI 2，请转到 [https://structure.io/openni](https://structure.io/openni) ，然后下载适用于 Windows 和系统体系结构（x64 或 x86）的最新 ZIP。 解压缩以获得安装文件，例如`OpenNI-Windows-x64-2.2.msi`。 运行安装程序。
...
Homebrew 没有提供安装具有 OpenNI 2 支持的 OpenCV 的选项。 Homebrew 始终使用`opencv_contrib`模块安装 OpenCV，包括非免费内容，例如获得专利的 SIFT 和 SURF 算法，我们将在[第 6 章](06.html)和*中检索图像和使用图像描述符搜索[* 。 如果您打算分发依赖于 OpenCV 的非免费内容的软件，则应自己调查专利和许可问题如何在特定国家和特定用例中应用。
...
包定义实际上是 Ruby 编程语言中的脚本。 可以在 Homebrew Wiki 页面上的 [https://github.com/Homebrew/brew/blob/master/docs/Formula-Cookbook.md](https://github.com/Homebrew/brew/blob/master/docs/Formula-Cookbook.md) 上找到有关编辑它们的提示。 脚本可以指定 Make 或 CMake 配置标志等。
...
可以在 [http://docs.opencv.org/](http://docs.opencv.org/) 中找到 OpenCV 的文档，您可以在其中在线阅读或下载以供离线阅读。 如果您在飞机上或其他无法上网的地方编写代码，则肯定要保留文档的离线副本。
