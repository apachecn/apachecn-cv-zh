...
您可以使用`model.getFLOPs`和`model.getMemoryConsumption`方法获得模型 FLOP 计数和消耗的内存量。 两种方法都将指定的BLOB形状作为输入。 每层推理时间统计信息在执行前向传递之后可用，并且可以通过`model.getPerfProfile`方法获得，该方法返回总推理时间和每层计时，所有信息均以滴答为单位。
