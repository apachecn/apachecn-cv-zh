...
以前，我们仅使用 OpenCV 的`cv2.imwrite`和`cv2.imread`功能分别保存和加载图像。 但是可以使用 NumPy 的数据持久性保存任何类型和形状的任何矩阵（不仅包含图像内容）。 在本秘籍中，我们将回顾如何做。
...
`cv2.bilateralFilter`功能提供了双边过滤。 它获取输入图像，窗口大小和颜色以及空间`σ`值。 如果窗口大小为负，则根据空间`σ`值计算得出。
