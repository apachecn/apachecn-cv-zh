...
最初，这个问题对寻找两个矩阵之间的正交变换的方式提出了质疑。 也许这与实际的计算机视觉应用无关，但是当您考虑到一组点确实是矩阵时，这种感觉可能会改变。 相机校准，刚体转换，摄影测量问题以及许多其他任务都需要解决正交 Procrustes 问题。 在本秘籍中，我们找到了估算点集旋转这一简单任务的解决方案，并研究了噪声输入数据如何影响我们的解决方案。
...
让我们还回顾一下前面代码中的 OpenCV 的另一个功能。 本书以前没有提到`cv2.getRotationMatrix2D`函数。 它为给定的旋转中心和角度以及比例尺计算仿射变换矩阵。 参数按以下顺序排列：旋转中心（格式为（`x`，`y`），旋转角度（以度为单位），比例。 返回的值是`2x3`仿射变换矩阵。
