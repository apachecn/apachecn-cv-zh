...
2.  现在，当用户按下`P`，`L`，`R`，`E`或`T`绘制点，线时， 矩形，椭圆形或文本。 另外，当用户按下`C`时，我们将清除图像，并在按下 *Esc* 键时关闭应用程序：
...
使用`cv2.VideoWriter`类执行视频编写。 构造函数采用输出视频路径**四个字符代码**（**FOURCC**），指定视频代码，所需的帧速率和帧大小。 编解码器代码的示例包括用于 MPEG-1 的`P`，`I`，`M`和`1`； 用于 Motion-JPEG 的`M`，`J`，`P`和`G`； 用于 XVID MPEG-4 的`X`，`V`，`I`和`D`； 和 H.264 的`H`，`2`，`6`和`4`。
