...
边缘是一种有用的图像特征，可以在许多计算机视觉应用程序中使用。 在本食谱中，您将学习如何使用 Canny 算法检测图像中的边缘。
...
在继续此食谱之前，您需要安装 OpenCV 3.x Python API 包和`matplotlib`包。
...
在本食谱中，我们将应用 Median Flow 对象跟踪器来跟踪视频中的对象。 该跟踪器可以实时工作（在现代硬件上甚至更快），并且可以准确，稳定地完成其工作。 另外，该跟踪器具有不错的功能，可以确定跟踪失败。 让我们看看如何在应用程序中使用它。

# 准备

在继续此食谱之前，您需要安装带有 OpenCV Contrib 模块的 OpenCV 3.x Python API 包。
...
在继续此食谱之前，您需要安装 OpenCV 3.x Python API 包和 matplotlib 包。 OpenCV 必须使用 Contrib 模块构建，因为跟踪 API 并非主要 OpenCV 存储库的一部分。
...
在本食谱中，您将学习如何检测棋盘和圆形网格图案。 这些模式在计算机视觉中非常有用，并且通常用于估计相机参数。
...
在本食谱中，您将学习如何使用具有 HOG 特征的预训练 SVM 模型来检测行人。 行人检测是许多**高级驾驶员辅助解决方案**（**ADAS**）的重要组成部分。 行人检测还用于视频监视系统和许多其他计算机视觉应用中。
...
在继续此食谱之前，您将需要安装 OpenCV 3.x Python API 包和`matplotlib`包。
...
在继续此食谱之前，您将需要安装 OpenCV 3.x Python API 包和 matplotlib 包。 OpenCV 必须使用 Contrib 模块构建，因为高级文本识别功能不是 OpenCV 主存储库的一部分。
...
在继续此食谱之前，您将需要安装 OpenCV 3.x Python API 包。
