...
本章视频中的配方“跟踪特征点”说明了如何使用`FrameProcessor`类接口。
...
该方法利用了其他四个实用程序方法。 您应该很容易地更改任何这些方法，以便为自己的跟踪器定义新的行为。 这些方法中的第一个检测特征点。 请注意，我们已经在第 8 章的第一个配方中讨论了`cv::goodFeatureToTrack`函数：
...
更好的策略是通过定期更新来动态构建背景模型。 这可以通过计算所谓的移动平均值（也称为**滑动平均值**）来实现。 这是一种计算时间信号平均值的方法，该方法考虑了收到的最新值。 如果 pt 是给定时间 t 的像素值，而μt-1 是当前平均值，则使用以下公式更新该平均值：
