...
为了定义图像中拐角的概念，哈里斯在假定的兴趣点周围的小窗口中查看了平均方向强度变化。 如果我们考虑位移向量`(u, v)`，则平均强度变化由下式给出：
...
`dx`和`dy`响应也用于估计特征的方向。 这些值是在半径为`6σ`的圆形邻域中，以规则的`σ`间隔定期计算的（内核尺寸为`4σ`）。 对于给定的方向，将某个角度间隔（`π / 3`）内的响应求和，并将给出最长向量的方向定义为主导方向。
