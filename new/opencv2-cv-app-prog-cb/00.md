...
*[第 1 章](01.html "Chapter 1. Playing with Images")* ，*播放图像*介绍了 OpenCV 库，并向您展示了如何使用 MS Visual C ++和 Qt 开发环境运行简单的应用程序。

*[第 2 章](02.html "Chapter 2. Manipulating the Pixels")* 和*操纵像素*解释了如何读取图像。 它描述了用于扫描图像以便对其每个像素执行操作的不同方法。 您还将学习如何在图像内定义关注区域。

*[第 3 章](03.html "Chapter 3. Processing Images with Classes")* 和*处理具有类*的图像，包含一些配方，这些配方提供了各种面向对象的设计模式，可以帮助您构建更好的计算机视觉应用程序。

*[第 4 章](04.html "Chapter 4. Counting the Pixels with Histograms")* 和*用直方图计算像素*展示了如何计算图像直方图以及如何使用它们来修改图像。 提出了基于直方图的不同应用程序，它们可以实现图像分割，目标检测和图像检索。

*[第 5 章](05.html "Chapter 5. Transforming Images with Morphological Operations")* 和*通过形态学操作变换图像*探索了数学形态学的概念。 它介绍了不同的运算符，以及它们如何用于检测图像中的边缘，角和线段。

*[第 6 章](06.html "Chapter 6. Filtering the Images")* 和*过滤图像*教您频率分析和图像过滤的原理。 它显示了如何将低通和高通滤波器应用于图像。 它提供了两个图像导数运算符：梯度和拉普拉斯算子。

*[第 7 章](07.html "Chapter 7. Extracting Lines, Contours, and Components")* 和*提取线，轮廓和分量*着重于几何图像特征的检测。 它说明了如何提取图像中的轮廓，线和连接的分量。

*[第 8 章](08.html "Chapter 8. Detecting and Matching Interest Points")* 和*检测和匹配兴趣点*描述了图像中的各种特征点检测器。 它还说明了兴趣点的描述符如何计算并用于匹配图像之间的点。

*[第 9 章](09.html "Chapter 9. Estimating Projective Relations in Images")* 和*估计图像中的投影关系*分析了图像形成中涉及的不同关系。 它还探讨了同一场景的两个图像之间存在的投影关系。

*[第 10 章](10.html "Chapter 10. Processing Video Sequences")* 和*处理视频序列*提供了读取和写入视频序列以及处理其帧的框架。 它还向您展示了如何逐帧跟踪特征点，以及如何提取在摄像机前移动的前景对象。
