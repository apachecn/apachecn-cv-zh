...
函数`cvLoadImage`是用于加载图像的 C 接口功能。 `cv::Mat`对象的构造器中的第二个参数指示该数据将不会被复制（如果想要新的副本，请将其设置为`true`，而`false`是默认值，因此可以将其省略）， `IplImage`和`image4`将共享相同的图像数据。 您需要在此处小心，不要创建悬空的指针。 因此，将`IplImage`指针封装到 OpenCV 2 提供的引用计数指针类中更为安全：
