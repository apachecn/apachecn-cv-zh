...
*还有更多... *的*部分，在[第 2 章](02.html "Chapter 2. Manipulating the Pixels")中介绍了邻居访问*，其中介绍了 `cv::filter2D`功能。 此功能可让您通过输入所选的内核将线性滤波器应用于图像。

# 使用中值过滤器过滤图像

本章的第一个配方介绍了线性滤波器的概念。 还存在可以有利地用于图像处理中的非线性滤波器。 这样的过滤器之一就是我们在本食谱中介绍的中值过滤器。

由于中值滤波器对于抵御椒盐噪声特别有用，因此我们将使用在[第 2 章](02.html "Chapter 2. Manipulating the Pixels")的第一个配方中创建的图像，该图像在此处复制：
...
配方*使用[第 7 章](07.html "Chapter 7. Extracting Lines, Contours, and Components")中的 Canny 运算符*检测边缘，其中通过使用两个不同的阈值获得二进制边缘图。
...
可以通过从图像中减去拉普拉斯算子来增强图像的对比度。 这是我们在[第 2 章](02.html "Chapter 2. Manipulating the Pixels")的食谱*中通过邻居访问*扫描图像的方法，我们在其中介绍了内核：
...
[第 8 章](08.html "Chapter 8. Detecting and Matching Interest Points")中的方法*检测尺度不变 SURF 特征*，使用拉普拉斯算子进行尺度不变特征的检测。