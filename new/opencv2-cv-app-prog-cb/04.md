...
您还可以定义一个查找表，以尝试改善图像的对比度。 例如，如果您观察到第一个配方中显示的上一张图像的原始直方图，则很容易注意到未使用整个范围的可能的强度值（特别是对于此图像，在图片中未使用较亮的强度值）。 因此，可以**拉伸**直方图，以产生具有扩大对比度的图像。 该程序旨在检测图像直方图中计数为非零的最低（`imin`）和最高（`imax`）强度值。 然后可以重新映射强度值，以使`imin`值重新定位为强度 0，并且为`imax`赋值 255。强度`i`之间的线性映射简单如下：
