...
本秘籍中介绍的类别`Histogram1D` 通过将 `cv::calcHist`功能限制为一维直方图来简化了功能。 这对于灰度图像很有用。 类似地，我们可以定义一个可用于计算彩色 BGR 图像直方图的类：
...
图像直方图使用可用的像素强度值捕获渲染场景的方式。 通过分析图像上像素值的分布，可以使用此信息来修改并可能改善图像。 本秘籍说明了如何使用由查找表表示的简单映射功能来修改图像的像素值。
