...
从[第 2 章](02.html "Chapter 2. Manipulating the Pixels")回忆起，OpenCV 矩阵和矢量数据结构包括基本算术运算符的定义，这也很有趣。 例如，如果要添加两个`cv::Vec3i`向量`a`和`b`并将结果分配给`c`，则可以简单地编写：
...
配方*使用[第 1 章](01.html "Chapter 1. Playing with Images")的 Qt* 创建 GUI 应用程序，以简要介绍 Qt GUI 框架及其信号和插槽模型。
...
使用平均移位算法至*的配方*在[第 4 章](04.html "Chapter 4. Counting the Pixels with Histograms")中找到对象*，该对象使用 HSV 颜色空间在图像中找到对象。*
