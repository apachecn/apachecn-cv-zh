...
您可能会注意到，我们通过调用`new`运算符创建了许多 Qt 对象，但从未删除它们。 很好，因为所有这些对象都是`QObject`的实例或其子类。 `QObject`的实例被组织在 Qt 库中的一个或多个对象树中。 当将`QObject`创建为另一个对象的子对象时，该对象将自动添加到其父对象的`children()`列表中。 父对象将获得子对象的所有权。 并且，当处置父对象时，其子对象将自动在其析构函数中删除。 在我们的应用中，我们将`QObject`的大多数实例创建为主窗口对象的子代，因此不需要删除它们。
