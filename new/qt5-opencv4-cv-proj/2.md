...
那就是在我们的应用中发生的事情-我们使用 Qt 加载图像并将其转换为 OpenCV `Mat`数据结构，然后对其进行处理并将其转换回`QImage`。 但是，如您所见，在使图像模糊时，我们没有交换红色和蓝色通道来求助于颜色顺序。 这是因为`blur`功能在通道上对称运行； 通道之间没有干扰，因此在这种情况下颜色顺序并不重要。 如果执行以下操作，则可以省略通道交换：
