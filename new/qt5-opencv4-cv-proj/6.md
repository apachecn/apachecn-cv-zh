...
如果您有很多猫的图片，那也可以。 `cv::VideoCapture`类的构造函数还接受由字符串表示的图像序列。 例如，如果将`"image_%02d.jpg"`字符串传递给构造函数，则`cv::VideoCapture`实例将读取名称类似于`image_00.jpg`，`image_01.jpg`和`image_02.jpg`等的图像，然后逐一显示为视频帧。
