...
我们将把波士顿公牛犬种的人脸作为目标。 `Images/n02096585-Boston_bull`目录中有 182 张波士顿公牛的图像。 与固定物体（例如交通标志）不同，我们找不到波士顿公牛队脸部的标准图片。 我们应该在刚刚选择的 182 张图像上注释狗的脸。 注释是使用 OpenCV v3.4.x 提供的`opencv_annotation`工具完成的：
