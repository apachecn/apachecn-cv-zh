...
让我们转到`glpanel.cpp`源文件以查看实现。 构造器和析构函数非常简单，因此在此不再赘述。 首先，让我们看一下初始化方法`void GLPanel::initializeGL()`：
