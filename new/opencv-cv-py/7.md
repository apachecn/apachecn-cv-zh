...
除 Mac 带有 Homebrew 的情况外，默认情况下，可执行文件的文件夹应位于`PATH`中。 对于 Homebrew，如果要将相关文件夹添加到`PATH`，请参阅第 1 章，“设置 OpenCV”的“使用 Homebrew 和现成的包（不支持深度摄像头）”第二部分中的说明。 否则，请注意可执行文件的完整路径，因为我们需要在运行它们时使用它。
...
`<cascade>`是与 OpenCV 的`CascadeClassifier`类的构造函数兼容的 XML 文件。 对于如何使用`CascadeClassifier`的示例，请参考第 4 章“用 Haar 级联跟踪人脸”的`FaceTracker`实现。 。 通过复制和修改`FaceTracker`和`Cameo`，您应该能够创建一个简单的测试应用，该应用在跟踪的自定义目标实例周围绘制矩形。
