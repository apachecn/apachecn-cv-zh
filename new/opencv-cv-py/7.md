...
`<cascade>`是与 OpenCV 的`CascadeClassifier`类的构造器兼容的 XML 文件。 对于如何使用`CascadeClassifier`的示例，请参考第 4 章“用 Haar 级联跟踪人脸”的`FaceTracker`实现。 。 通过复制和修改`FaceTracker`和`Cameo`，您应该能够创建一个简单的测试应用，该应用在跟踪的自定义目标实例周围绘制矩形。
