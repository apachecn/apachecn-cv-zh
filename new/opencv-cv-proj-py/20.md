...
最后一步是将三角剖分的 3D 现实世界点可视化。 创建 3D 散点图的一种简单方法是使用 matplotlib。 但是，如果您正在寻找更专业的可视化工具，则可能对 Mayavi （ [http://docs.enthought.com/mayavi/mayavi](http://docs.enthought.com/mayavi/mayavi) ），VisPy （ [http://vispy.org](http://vispy.org) 或点云库（ [http://pointclouds.org](http://pointclouds.org) ）。 尽管后者尚不支持点云可视化的 Python，但它是用于点云分割，过滤和样本一致性模型拟合的出色工具。 有关更多信息，请访问[这个页面](https://github.com/strawlab/python-pcl)上的 strawlab 的 GitHub 信息库。
