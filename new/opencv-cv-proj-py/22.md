...
在本章中，我们将训练**支持向量机**（**SVM**）来识别各种交通标志。 尽管 SVM 是二分类器（也就是说，它们最多可用于学习两类：正负项，动物和非动物等），但它们可以扩展以用于**多类**分类。 为了获得良好的分类性能，我们将探索许多颜色空间以及定向梯度直方图（**HOG**）特征。 然后，将基于**准确率**，**精度**和**召回**来判断分类性能。 以下各节将详细解释所有这些术语。
...
4.  **为分类器评分**：我们将通过计算不同的性能指标（例如准确率，准确率和召回率）来评估经过训练的集成分类器的质量。
...
*   `classifiers.MultiClassSVM`：使用以下公共方法为多类分类实现 SVM 集成的类：
    ...
    *   `MultiClassSVM.evaluate`：一种用于通过在训练后将 SVM 应用于某些测试数据来评估 SVM 整体的方法。 它以测试数据矩阵作为输入，其中每一行都是测试样本，各列包含特征值和标签向量。 该函数返回三种不同的性能指标：准确率，准确率和召回率。
...
平均减法是最常见的预处理形式（有时也称为**零中心化**或**去均值**），其中对数据集中的所有样本计算每个特征维的平均值。 然后，从数据集中的每个样本中减去此按功能划分的平均值。 您可以认为此过程将数据的*点云*集中在原点上。 归一化是指数据维度的缩放比例，以便它们具有大致相同的比例。 这可以通过以下方式实现：将每个维度除以其标准偏差（一旦它已为零中心），或者将每个维度缩放到`[-1, 1]`的范围内。 仅当您有理由相信不同的输入特征具有不同的比例或单位时，才应应用此步骤。 在图像的情况下，像素的相对比例已经大约相等（并且在`[0, 255]`的范围内），因此不必严格执行此附加预处理步骤。
...
现在，我们已经准备好来计算所需的性能指标，这将在后面的部分中详细介绍。 就本章而言，我们选择计算准确率，精确度和召回率，它们以自己专用的私有方法实现：
...
对于这些设置中的每一个，我们需要收集三个性能指标—准确率，精度和召回率：
...
这样就结束了嵌套的`for`循环。 剩下要做的就是可视化结果。 为此，我们选择 Matplotlib 的条形图功能。 目标是显示提取的特征和分类策略的所有组合的三个性能指标（准确率，准确率和召回率）。 我们将为每个分类策略使用一个绘图窗口，并在堆叠的条形图中排列相应的数据：
