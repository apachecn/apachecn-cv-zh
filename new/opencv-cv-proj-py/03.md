...
OpenCV 提供了一个非常通用的函数`filter2D()`和，它可以应用我们指定的任何核或卷积矩阵。 要了解如何使用此功能，让我们首先学习卷积矩阵的格式。 它是一个二维数组，具有奇数行和列。 中心元素对应于关注像素，其他元素对应于该像素的邻居。 每个元素都包含一个整数或浮点值，该整数或浮点值是应用于输入像素值的权重。 考虑以下示例：
