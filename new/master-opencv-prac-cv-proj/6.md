...
非刚性人脸跟踪是视频流每一帧中一组准密集的面部特征的估计，这是一个难题，现代方法从许多相关领域借鉴了思想，包括计算机视觉，计算几何 ，机器学习和图像处理。 这里的非刚性指的是以下事实：面部特征之间的相对距离在面部表情和整个人群之间变化，并且不同于面部检测和跟踪，后者仅旨在在每个帧中查找面部的位置，而不是配置 面部特征 非刚性人脸跟踪是一个流行的研究主题，已经有二十多年的历史了，但是直到最近，各种方法才变得足够鲁棒，处理器也足够快，这使得构建商业应用成为可能。
...
开发面部跟踪系统的阻碍因素之一是人工注释大量图像的繁琐且容易出错的过程，每个图像都有很多点。 为了简化此过程，以遵循本章中的工作，可以从以下位置下载公开可用的 [MUCT 数据集](http:// http://www/milbo.org/muct)。
...
请注意，EigenFace和支持向量机方法最初是为分类而不是检测或图像对齐而开发的。 但是，它们的基本数学概念已显示适用于面部跟踪领域。
