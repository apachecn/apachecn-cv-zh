...
**本章算法的概述**：给定一个图像（前面的屏幕快照中的左上方图像），我们可以使用主动外观搜索算法来找到人头的 2D 姿势。 屏幕快照的右上图显示了在搜索算法中使用的先前训练的 Active Appearance 模型。 找到姿势后，可以应用 POSIT 将结果扩展到 3D 姿势。 如果将该程序应用于视频序列，则将通过检测获得 3D 跟踪。
...
为什么要完全使用 PCA？ 因为 PCA 在减少模型参数数量方面将真正为我们提供帮助。 在本章的后面，我们还将看到在给定图像中进行搜索时有多大帮助。 应当为以下引用提供一个网页 URL（ [http://en.wikipedia.org/wiki/Principal_component_analysis](http://en.wikipedia.org/wiki/Principal_component_analysis) ）：
...
图片来源： [http://en.wikipedia.org/wiki/File:GaussianScatterPCA.png](http://en.wikipedia.org/wiki/File:GaussianScatterPCA.png)
...
尽管细分创建是 OpenCV 的一个非常方便的功能，但是遍历所有三角形可能并不容易。 以下代码显示了如何遍历细分的边缘：
...
前面的屏幕截图显示了通过在另一个图像上进行主动外观模型实例化而获得的闭合的嘴巴。 它显示了如何将微笑的嘴巴和钦佩的脸相结合，从而推断出训练过的图像。

前面的屏幕快照是通过仅更改形状的三个参数和纹理的三个参数而获得的，这是 AAM 的目标。 已经开发了一个示例应用程序，可以在 [http://www.packtpub.com/](http://www.packtpub.com/)上找到，供读者试用 AAM。 实例化新模型只是滑动方程参数的问题，如在*获得 PCA* 的感觉一节中所定义。 您应该注意，AAM 搜索和拟合依靠这种灵活性来找到与训练模型不同位置的给定捕获模型的最佳匹配。 我们将在下一部分中看到这一点。
...
*   确保使用通过校准过程获得的相机焦距。 您可能需要检查[第 2 章](2.html "Chapter 2. Marker-based Augmented Reality on iPhone or iPad")和*基于标记的增强现实在 iPhone 或 iPad 上的*相机校准*部分中可用的校准程序之一。 当前 POSIT 的实现仅允许正方形像素，因此在 x 和 y 轴上没有焦距的空间。*
...
为了将 POSIT 用作头部姿势的工具，您将需要使用 3D 头部模型。 科英布拉大学系统与机器人研究所提供了一种，可以在 [http://aifi.isr.uc.pt/Downloads/OpenGL/glAnthropometric3DModel.cpp](http://aifi.isr.uc.pt/Downloads/OpenGL/glAnthropometric3DModel.cpp) 中找到。 请注意，可以从以下位置获得模型：
...
现在，所有工具都已经组装好，可以进行 6 个自由度的头部跟踪，我们可以将其应用于摄像机流或视频文件。 OpenCV 提供了`VideoCapture`类，可以按以下方式使用（请参阅[第 1 章](1.html "Chapter 1. Cartoonifier and Skin Changer for Android")， *Cartoonifier 和 Skin Changer for Android* 中的*访问网络摄像头*部分。 ， 更多细节）：
...
加载完所需的所有内容后，我们需要从已知的姿态（即已知的 3D 位置，已知的旋转度和已知的 AAM 参数集）初始化算法。 这可以通过 OpenCV 高度记录的 Haar 特征分类器人脸检测器自动完成（更多信息，请参见[第 6 章](6.html "Chapter 6. Non-rigid Face Tracking")，*，非刚性人脸跟踪*的*人脸检测*部分）， 或在 OpenCV 的级联分类器文档中），或者我们可以从先前注释的帧中手动初始化姿势。 也可以使用蛮力方法，即为每个矩形运行 AAM 拟合，因为这种方法仅在第一帧期间会非常慢。 请注意，通过初始化，我们的意思是通过其参数找到 AAM 的 2D 界标。
...
经过足够多的 AAM 理论和实践，我们深入研究了 POSIT，以便将 2D 测量与 3D 测量耦合起来，说明如何使用模型点之间的匹配来拟合 3D 模型。 在本章的结尾，我们展示了如何通过检测来使用在线面部跟踪器中的所有工具，从而产生 6 个自由度的头部姿势（旋转 3 度）和 3 个平移姿势。 可以从 [http://www.packtpub.com/](http://www.packtpub.com/) 下载本章的完整代码。

# 参考

*   *有效外观模型*， *T.F. 老兄*， *GJ Edwards* ，*和* *CJ Taylor* ， *ECCV* ， *2：484–498* ， *1998* （ [http://www.cs.cmu.edu/~efros/courses/AP06/Papers/cootes-eccv-98.pdf](http://www.cs.cmu.edu/~efros/courses/AP06/Papers/cootes-eccv-98.pdf) ）
*   *主动形状模型-训练和应用*， *老兄*， *CJ Taylor* ， *DH Cooper* ，*和* *J. Graham* ，*计算机视觉和图像理解*，*（61）：38-59* ， *1995* （ [http://www.wiau.man.ac.uk/~bim/Papers/cviu95.pdf](http://www.wiau.man.ac.uk/~bim/Papers/cviu95.pdf) ）
*   *MUCT 具有里程碑意义的人脸数据库*， *S. Milborrow* ， *J. Morkel* 和 *F. Nicolls* ，*模式识别协会 南非*， *2010* （ [http://www.milbo.org/muct/](http://www.milbo.org/muct/) ）
*   *IMM 人脸数据库-240 张人脸图像的注释数据集*， *Michael M. Nordstrom* ， *Mads Larsen* ， *Janusz Sierakowski* ，*和* *Mikkel B. Stegmann* ，*信息学和数学建模*，*丹麦技术大学*， *2004* ，（ [http://www2.imm.dtu.dk/~aam/datasets/datasets.html](http://www2.imm.dtu.dk/~aam/datasets/datasets.html) ）
*   *网上视频*， *B. Delaunay* ， *Izvestia Akademii Nauk SSSR* ， *Otdelenie Matematicheskikh i Estestvennykh Nauk* ， *7 –* ， *1934*
*   *面部表情识别和单眼头部姿势估计主论文的主动出现模型*， *P. Martins* ， *2008*
*   *主动外观模型再访*，*国际计算机视觉杂志*，*卷。 60* ， *2 号*，*第 135-164 页*， *I. Mathews* *和* *S. Baker* ， *11 月*， *2004* （ [http://www.ri.cmu.edu/pub_files/pub4/matthews_iain_2004_2/matthews_iain_2004_2.pdf](http://www.ri.cmu.edu/pub_files/pub4/matthews_iain_2004_2/matthews_iain_2004_2.pdf) ）
*   *POSIT 教程*， *Javier Barandiaran* （ [http://opencv.willowgarage.com/wiki/Posit](http://opencv.willowgarage.com/wiki/Posit) ）
*   *25 行代码中基于模型的对象姿势，国际计算机视觉杂志*， *15* ，*第 123-141 页*， *Dementhon* *和* *LS Davis* ， *1995* （ [http://www.cfar.umd.edu/~daniel/daniel_papersfordownload/Pose25Lines.pdf](http://www.cfar.umd.edu/~daniel/daniel_papersfordownload/Pose25Lines.pdf) ）