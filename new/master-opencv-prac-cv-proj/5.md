...
在本章中，我们将使用西班牙的车牌。 在西班牙，有三种不同尺寸和形状的车牌； 我们将仅使用最常见的（大型）车牌`520 x 110`毫米。 两组字符之间相隔 41 毫米，然后 14 毫米宽将每个字符分开。 第一组字符有四个数字，第二组字符有三个字母，没有元音 A，E，I，O，U，也没有字母Ñ或 Q。 所有字符的尺寸均为`520 x 110`毫米。
...
在 Sobel 过滤器之后，我们应用阈值过滤器来获得具有通过 Otsu 方法获得的阈值的二进制图像。 Otsu 的算法需要 8 位输入图像，而 Otsu 的方法会自动确定最佳阈值：
