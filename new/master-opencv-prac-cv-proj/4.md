...
在下面的图像中，我们将在 [http://cvlab.epfl.ch/~strecha/multiview/denseMVS.html](http://cvlab.epfl.ch/~strecha/multiview/denseMVS.html) 上看到来自 Fountain P-11 序列的两个图像的三角剖分结果。 顶部的两个图像是场景的原始两个视图，底部的一对是从这两个视图重建的点云的视图，包括估计的注视着喷泉的摄像机。 我们可以看到红砖墙壁的右侧部分是如何重建的，还有从墙壁突出的喷泉。
