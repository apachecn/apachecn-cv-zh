...
通过调用`calcOpticalFlowPyrLK`函数，在 OpenCV 中使用光流相当容易。 但是，我们希望保持 OF 的结果匹配与使用丰富功能的结果相似，因为将来我们希望这两种方法可以互换。 为此，我们必须安装一种特殊的匹配方法，该方法可以与以前的基于功能的方法互换，该方法将在下面的代码部分中看到：
