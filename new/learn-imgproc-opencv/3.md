...
OpenCV 提供了四种使用非局部均值方法对彩色和灰度图像进行降噪的功能。 对于 TVL1 模型，提供了一种功能。 这些功能为：
...
*   `void fastNlMeansDenoisingColored(InputArray src, OutputArray dst, float h = 3, float hForColorComponents = 3, int templateWindowSize = 7, int searchWindowSize = 21)`：这是对彩色图像先前功能的修改。 它将`src`图像转换为 CIELAB 色彩空间，然后使用`fastNlMeansDenoising`函数分别对 L 和 AB 分量进行降噪。
...
*   `void fastNlMeansDenoisingColoredMulti(InputArrayOfArrays srcImgs, OutputArray dst, int imgToDenoiseIndex, int temporalWindowSize, float h = 3, float hForColorComponents = 3, int templateWindowSize = 7, int searchWindowSize = 21)`：基于`fastNlMeansDenoisingColored`和`fastNlMeansDenoisingMulti`函数。 这些参数将在其余功能中说明。
