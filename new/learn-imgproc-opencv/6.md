...
我们如何估计相机响应功能？ 我们从像素本身做到这一点！ 响应函数是每个颜色通道的 Sigmoid 曲线，可以根据像素进行估算（如果有 3 个像素曝光，则每个颜色通道的曲线上有 3 个点）。 由于这非常耗时，因此通常选择一组随机像素。
...
OpenCV 提供了 G. Ward 在 2003 年提出的图像对齐算法。主要函数`createAlignMTB`采用定义最大位移的输入参数（实际上，每个尺寸的最大位移以 2 为底的对数）。 在上一示例中估算摄像机响应功能之前，应插入以下几行：
