...
例
 ...
 |
| --- | --- | --- |
| （小）矢量 | `VecAB`，其中`A`可以是 2、3、4、5 或 6，`B`可以是`b`，`s`，`i`，`f`或`d` | 
...
阅读并显示输入的图像后，我们通过绘制一个填充的白色圆圈来创建蒙版。 在 AND 操作中，使用此掩码。 逻辑运算仅适用于掩码值不为零的像素； 其他像素不受影响。 最后，在此示例中，我们用白色填充结果图像的外部（即，圆的外部）。 使用前面说明的像素访问方法之一完成此操作。 在以下屏幕截图中查看生成的图像：
...
*   `const float** ranges`：此参数是每个维度中直方图 bin 边界的 dims 数组。
...
*   **源图像**：这是下图中左上角的。
*   **均匀彩色图像**：这是下图中右上角显示的。
*   **三个通道的直方图**：对于源图像，此处 R =读取，G =绿色，B =蓝色。 下图的左下角显示了该内容。
*   **均衡图像**的 RGB 通道的直方图：在下图右下角中显示。 该图显示了由于均衡过程如何延长了 R，G 和 B 的最频繁强度值。
...
最后，您可以参考[第 3 章](3.html "Chapter 3. Correcting and Enhancing Images")，*校正和增强图像*以及其中的示例，以涵盖此广泛主题的基本方面，例如图像 通过直方图建模进行增强。

### 注意

有关更多信息，请参阅 Packt Publishing 的 *OpenCV Essentials* ，Deniz O.，FernándezMM，VállezN.，Bueno G.，Serrano I.，Patón.A。，Salido J.，Packt Publishing， [https ：//www.packtpub.com/application-development/opencv-essentials](https://www.packtpub.com/application-development/opencv-essentials) 。

# 摘要

本章涵盖并建立了应用计算机视觉中使用的图像处理方法的基础。 图像处理通常是进一步实现计算机视觉应用的第一步，因此，这里已涉及到：基本数据类型，像素级访问，图像的常规操作，算术运算，数据持久性和直方图。

您还可以参考 Packt Publishing 的 *OpenCV Essentials* 的[第 3 章](3.html "Chapter 3. Correcting and Enhancing Images")，*校正和增强图像*，以涵盖该广泛主题的其他重要方面，例如图像 增强，通过滤波的图像恢复以及几何校正。
