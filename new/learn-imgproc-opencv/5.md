...
第四步，选择图像并匹配子集以构建全景图。 然后，使用`vector<int> indices = leaveBiggestComponent(features, pairwise_matches, conf_thresh)`函数选择并匹配最佳功能。 通过这些功能，将创建一个新的子集以供使用。
