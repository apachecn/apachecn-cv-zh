...
OpenCV 可从 [http://opencv.org](http://opencv.org) 免费下载。 该站点提供了发行的最新版本（当前为 3.0 beta）和较旧的版本。

### 注意

如果下载的版本是不稳定版本，例如当前的 3.0 beta 版本，则应格外小心，以免出现错误。

在 [http://opencv.org/downloads.html](http://opencv.org/downloads.html) 上，可以找到适用于每个平台的 OpenCV 版本。 可以根据最终目的从不同的存储库中获取代码和库信息：

*   主存储库（位于 [http://sourceforge.net/projects/opencvlibrary](http://sourceforge.net/projects/opencvlibrary) ），专用于最终用户。 它包含库的二进制版本和目标平台的可编译源。
*   测试数据存储库（位于 [https://github.com/itseez/opencv_extra](https://github.com/itseez/opencv_extra) ），其中包含用于测试某些库模块目的的数据集。
*   The contributions repository (at [http://github.com/itseez/opencv_contrib](http://github.com/itseez/opencv_contrib)) with the source code corresponding to extra and cutting-edge features supplied by contributors. This code is more error-prone and less tested than the main trunk.

    ### 提示

    在最新版本的 OpenCV 3.0 beta 中，额外的贡献模块未包含在主软件包中。 它们应单独下载，并通过适当的选项明确包含在编译过程中。 如果包括其中一些贡献的模块，请务必谨慎，因为其中一些模块依赖于 OpenCV 不附带的第三方软件。

*   每个模块的文档站点（位于 [http://docs.opencv.org/master/](http://docs.opencv.org/master/) ），包括提供的模块。
*   开发库（位于 [https://github.com/Itseez/opencv](https://github.com/Itseez/opencv) ），带有库的当前开发版本。 它适用于库主要功能的开发人员以及希望在发布最新版本之前仍使用最新更新的“急躁”用户。
...
使用 OpenCV 最快的方法是使用发行版随附的预编译版本之一。 然后，更好的选择是使用用于软件开发的本地平台的最佳设置来构建库的微调版本。 本章提供在 Windows 上构建和安装 OpenCV 的信息。 在 [http://docs.opencv.org/doc/tutorials/introduction/linux_install](http://docs.opencv.org/doc/tutorials/introduction/linux_install) 和 [https://help.ubuntu.com/community 上可以找到在 Linux 上设置库的更多信息。 / OpenCV](https://help.ubuntu.com/community/OpenCV) 。

## 获取编译器并设置 CMake

使用 OpenCV 开发跨平台的一个不错的选择是使用 **GNU 工具包**（包括 gmake，g ++和 gdb）。 对于大多数流行的操作系统，可以轻松获得 GNU 工具包。 对于开发环境，我们的首选选择包括 GNU 工具包和跨平台 **Qt 框架**，其中包括 Qt 库和 Qt Creator **集成开发环境** **（IDE）**。 Qt 框架可从 [http://qt-project.org/](http://qt-project.org/) 免费获得。

### 注意

在 Windows **和**上安装编译器之后，请记住正确设置`Path`环境变量，为编译器的可执行文件添加路径，例如 Qt 框架随附的 GNU /编译器的`C:\Qt\Qt5.2.1\5.2.1\mingw48_32\bin`。 在 Windows 上，免费的**快速环境编辑器**工具（可从 [http://www.rapidee.com](http://www.rapidee.com) 获得）提供了一种方便的方式来更改`Path`和其他环境变量 。

要以与编译器无关的方式管理 OpenCV 库的生成过程，推荐使用 CMake 工具。 CMake 是可从 [http://www.cmake.org/](http://www.cmake.org/) 上获得的免费且开源的跨平台工具。
...
安装 OpenCV 的简短说明适用于 **Windows** 。 可以在 [http://docs.opencv.org/doc/tutorials/introduction/linux_install/linux_install.html](http://docs.opencv.org/doc/tutorials/introduction/linux_install/linux_install.html) 上阅读有关 **Linux** 前提条件的详细说明。 尽管本教程适用于 OpenCV 2.0，但几乎所有信息对于版本 3.0 仍然有效。
...
OpenCV 具有模块化的结构，这意味着该程序包为每个模块都包含一个静态或动态（DLL）库。 每个模块的正式文档可以在 [http://docs.opencv.org/master/](http://docs.opencv.org/master/) 中找到。 软件包中包含的主要模块是：
...
从 3.0 beta 版开始，新的贡献模块以单独的软件包（`opencv_contrib-master.zip`）分发，可以从 [https://github.com/itseez/opencv_contrib](https://github.com/itseez/opencv_contrib) 下载。 这些模块提供了的附加功能，在使用它们之前，应充分了解它们。 有关新版 OpenCV（版本 3.0）中新功能的快速概述，请参考位于 [http://opencv.org/opencv-3-0-beta.html](http://opencv.org/opencv-3-0-beta.html) 的文档。
...
您可以在 [https://gcc.gnu.org/onlinedocs/](https://gcc.gnu.org/onlinedocs/) 和 [https：// www 中找到 GNU GCC 可用在线文档的完整列表，并制作。 gnu.org/software/make/manual/](https://www.gnu.org/software/make/manual/) 。
...
对于 Windows，可以从 [http://qt-project.org/](http://qt-project.org/) 下载 Qt 捆绑包（包括 Qt 库，Qt Creator 和 MinGW 套件）。 捆绑包约为 700 MB。
...
有关在 Qt 项目中开发的工具（包括 Qt Creator 和 **qmake** ）的详细说明，请访问 [http://doc.qt.io/](http://doc.qt.io/) 。
...
有关编解码器属性的详细列表，请查看 [http://docs.opencv.org/master/modules/refman.html](http://docs.opencv.org/master/modules/refman.html) 上的`imgcodecs.hpp`头文件和 OpenCV API 参考。 。
...
*   `static int VideoWriter::fourcc(char c1, char c2, char c3, char c4)`：这会将四个字符连接到一个 **fourcc** 代码。 在示例中，MSVC 代表 Microsoft Video（仅适用于 Windows）。 有效的 fourcc 代码列表发布在 [http://www.fourcc.org/codecs.php](http://www.fourcc.org/codecs.php) 上。
*   `bool VideoWriter::isOpened()`: This returns `true` if the object for writing the video was successfully initialized. For instance, using an improper codec produces an error.
    ...
    要小心; 系统中有效的 fourcc 代码取决于本地安装的编解码器。 要了解本地系统中已安装的 fourcc 编解码器，我们建议使用开源工具 MediaInfo，该工具可在 [http://mediaarea.net/en/MediaInfo](http://mediaarea.net/en/MediaInfo) 上的许多平台上使用。
