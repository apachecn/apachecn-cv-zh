...
*   **OpenCV 头文件和库二进制文件**：当然，我们需要编译 OpenCV，辅助库是进行此类编译的前提条件。 该软件包应使用与生成用户应用相同的编译器进行编译。
...
上一个文件说明了`qmake`生成适当的 Makefile 来构建项目二进制文件所需的选项。 每行以一个标记（表示`TARGET`，`CONFIG`，`SOURCES`，`INCLUDEPATH`和`LIBS`）的标签开头，后跟一个标记以添加（`+=`）或删除（`-=`） 期权的价值。 在此示例项目中，我们使用非 Qt 控制台应用。 可执行文件为`showImage.exe`（`TARGET`），源文件为`showImage.cpp`（`SOURCES`）。 由于此项目是基于 OpenCV 的应用，因此最后两个标签指示此特定项目（`core`，`imgcodecs`，`highgui`和`imgproc`）。 注意，在行末尾的反斜杠表示在下一行继续。
