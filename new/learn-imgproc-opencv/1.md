...
通过将库二进制文件的位置（例如，在 **Windows** 中，生成的 **DLL** 文件位于`OPENCV_BUILD\install\x64\mingw\bin`）添加到**路径**，安装环境变量的末尾。 如果 **Path** 字段中没有此目录，则每个 OpenCV 可执行文件的执行都会出错，因为找不到库二进制文件。
...
轨迹栏是在指定窗口（`winname`）中使用`createTrackbar(const String& trackbarname, const String& winname, int* value, int count, TrackbarCallback onChange=0, void* userdata=0)`功能创建的，具有链接的整数值（`value`），最大值（`count`）和可选的**滑块更改时将调用 callback** 函数（`onChange`），以及 callback 函数的参数（`userdata`）。 回调函数本身有两个参数：`value`（由滑块选择）和一个​​指向`userdata`的指针（可选）。在 Qt 支持下，如果未指定窗口，则是属性窗口。 在 **showUI** 示例中，我们创建了两个跟踪栏：第一个在主窗口中，第二个在属性窗口中。 跟踪栏回调的代码为：
