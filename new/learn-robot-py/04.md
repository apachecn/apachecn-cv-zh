...
*   **插入**：**插入**选项卡允许您向场景添加新的仿真模型。 这些模型在本地系统和远程服务器中均可用。 `/home/<user_name>/.gazebo/model`文件夹会将本地模型文件和模型保存在 [http://gazebosim.org/models](http://gazebosim.org/models) 中的远程服务器中，如以下屏幕快照所示：
...
您可以从 [http://gazebosim.org/tutorials?cat=guided_b & amp; tut = guided_b2](http://gazebosim.org/tutorials?cat=guided_b&tut=guided_b2) 获取有关凉亭 GUI 的更多信息。
...
请参阅 [http://gazebosim.org/tutorials/?tut=ros_urdf](http://gazebosim.org/tutorials/?tut=ros_urdf) 以查看模拟中使用的所有标签。
...
有关更多详细信息，请参见 [http://wiki.ros.org/gmapping](http://wiki.ros.org/gmapping) 上的 ROS Gmapping Wiki 页面。
