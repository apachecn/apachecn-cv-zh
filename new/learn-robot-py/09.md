...
前面的命令将为 UI 文件生成等效的 Python 代码。 这将创建一个具有 GUI 组件的 Python 类。 生成的脚本将没有自定义函数`message()`的定义。 我们应该添加此自定义函数来生成代码。 以下过程将指导您添加自定义功能； 因此，当您单击按钮时，将执行自定义函数`message()`。
...
此处提供了先前功能中使用的`WorkThread()`类。 `WorkThread()`类继承自 Qt 提供的用于线程化的`QThread`。 线程仅以特定的延迟发出信号`update(Qstring)`。 在先前的函数`update_values()`中，`update(QString)`信号连接到`self.add()`插槽； 因此，从线程发出信号`update(QString)`时，它将调用`add()`插槽并更新电池和状态值：
