...
`threshold_otsu(gray_image)`函数基于大津的二值化算法返回阈值。 然后，使用该值构造二进制图像（`dtype= bool`），应将其转换为 8 位无符号整数格式（`dtype= uint8`）以进行适当的可视化。 `img_as_ubyte()`功能用于此目的。 该示例的完整代码可以在`thresholding_scikit_image_otsu.py`脚本中看到。
