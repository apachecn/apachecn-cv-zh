...
*   [第 1 章](../Text/01.html)，*设置 OpenCV* ：`opencv-contrib-python`
*   [第 2 章](../Text/02.html)， *OpenCV 中的图像基础*：`opencv-contrib-python`和`matplotlib`
*   [第 3 章](../Text/03.html)，*处理文件和图像*：`opencv-contrib-python`和`matplotlib`
*   [第 4 章](../Text/04.html)，*在 OpenCV 中构建基本形状*：`opencv-contrib-python`和`matplotlib`
*   [第 5 章](../Text/05.html)和*图像处理技术*：`opencv-contrib-python`和`matplotlib`
*   [第 6 章](../Text/06.html)和*直方图的构建和构建*：`opencv-contrib-python`和`matplotlib`
*   [第 7 章](../Text/07.html)，*阈值处理技术*：`opencv-contrib-python`，`matplotlib`，`scikit-image`和`scipy`
*   [第 8 章](../Text/08.html)，*轮廓检测，过滤和绘制*：`opencv-contrib-python`和`matplotlib`
*   [第 9 章](../Text/09.html)，*增强现实*：`opencv-contrib-python`和`matplotlib`
*   [第 10 章](../Text/10.html)，*使用 OpenCV 进行机器学习*：`opencv-contrib-python`和`matplotlib`
*   [第 11 章](../Text/11.html)，*人脸检测，跟踪和识别*：`opencv-contrib-python`，`matplotlib`，`dlib`，`face-recognition`，`cvlib`，`requests`，`progressbar` ，`keras`和`tensorflow`
*   [第 12 章](../Text/12.html)，*深度学习简介*：`opencv-contrib-python`，`matplotlib`，`tensorflow`和`keras`
*   [第 13 章](../Text/13.html)，*使用 Python 和 OpenCV 的移动和 Web 计算机视觉*：`opencv-contrib-python`，`matplotlib`，`flask`，`tensorflow`，`keras`，`requests`和 `pillow`
...
*   **SciPy** （ [https://www.scipy.org/](https://www.scipy.org/) ）是一个用于科学和技术计算的库。
...
可以从以下 GitHub 存储库下载可以使用的模板： [https://github.com/dbader/readme-template](https://github.com/dbader/readme-template) 。 有关更多信息，请参见 [https://dbader.org/blog/write-a-great-readme-for-your-github-project](https://dbader.org/blog/write-a-great-readme-for-your-github-project) 。
