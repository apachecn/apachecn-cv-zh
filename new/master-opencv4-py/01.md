...
我们可以看到，调用此方法的结果是一条消息显示在屏幕上。 此方法是一种简单的方法，它知道所有内容均已正确安装，因为它涉及到安装，导入和使用包中的功能。 此外，我们可以调用`helloopencv`包中包含的更有用的方法。 例如，您可以调用`load_image`方法从磁盘加载映像，然后，可以使用`show_image`方法显示它：
