...
`attempts`参数指定使用不同的初始标签执行算法的次数。 该算法返回产生最佳紧密度的标签。 `flags`参数指定如何获取初始中心。 `cv2.KMEANS_RANDOM_CENTERS`标志在每次尝试中选择随机的初始中心。 `cv2.KMEANS_PP_CENTERS`标志使用 Arthur 和 Vassilvitskii 提出的 k-means ++中心初始化（请参阅《K-Means++：精心播种的优势》（2007））。
