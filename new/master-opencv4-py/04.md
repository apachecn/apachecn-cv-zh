...
有关如何安装它们的更多详细信息，请参见[第 1 章](../Text/01.html)，*设置 OpenCV* 。 可以在此处访问 GitHub 存储库，该存储库包含从第一章到最后一章都需要完成本书的所有支持项目文件： [https://github.com/PacktPublishing/Mastering-OpenCV-4-with-Python](https://github.com/PacktPublishing/Mastering-OpenCV-4-with-Python) 。
...
如果您不知道这些数字是什么或代表什么，则应阅读[第 2 章](../Text/02.html)和 *OpenCV* 中的图像基础知识，在此介绍了这些概念。
...
如您所见，由`p1`和`p2`点定义的线段显示为黄色，*将*原始线段剪切为矩形。 在这种情况下，`ret`为`True`，因为至少有一个点在矩形内，这就是绘制由`pt1`和`pt2`定义的黄色部分的原因。
