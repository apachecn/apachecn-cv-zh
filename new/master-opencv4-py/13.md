...
导入所需的包后，我们将创建`Flask`类的实例，该实例将成为我们的 **Web 服务器网关接口**（**WSGI**）应用。 `route()`装饰器用于指示哪个 URL 应该触发`hello()`函数，该功能将打印消息`Hello World!`。
...
此功能准备输入图像-将图像转换为 RGB，调整其大小，将一个或多个图像转换为批量格式，最后使用特定的架构对其进行预处理。
