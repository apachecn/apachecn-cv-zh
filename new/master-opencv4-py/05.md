...
可以将`cv2.bilateralFilter()`功能应用于输入图像，以便应用双边过滤器。 此功能可用于减少噪声，同时保持边缘清晰，如以下代码所示：
