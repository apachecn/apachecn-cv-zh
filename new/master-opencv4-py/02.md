...
此外，其他 Python 程序包使用 RGB 颜色格式。 因此，我们需要知道如何将图像从一种格式转换为另一种格式。 例如，Matplotlib 使用 RGB 颜色格式。 [Matplotlib](https://matplotlib.org/) 是最受欢迎的 2D Python 绘图库，可为您提供多种绘图方法。 您可以与绘制的图像进行交互（例如，放大并保存图像）。 Matplotlib 可以在 Python 脚本或 Jupyter Notebook 中使用。 [您可以查看 Matplotlib 文档以获取更多详细信息](https://matplotlib.org/contents.html)。
...
最后，您可以开始执行加载的笔记本文档。 您可以通过按`Shift + Enter`来逐步执行笔记本（一次一个单元）。 另外，您可以通过单击“单元格” |“步骤”一步来执行整个笔记本。 运行全部菜单。 此外，您还可以通过单击 Kernel | Kernel |重新启动内核（计算引擎）。 重新启动菜单。
