...
此函数接收直方图，并为直方图的每个元素建立`(x, y)`点，`pts`点，其中``y``值表示`x`元素的频率 直方图的 这些点`pts`是使用`cv2.polylines()`函数绘制的，我们已经在第 4 章，“在 OpenCV 中构造基本形状”。 此函数基于`pts`阵列绘制多边形曲线。 最后，由于`y`值上下颠倒，因此图像垂直翻转。 在下一个屏幕截图中，我们可以使用`plt.plot()`和我们的自定义功能比较可视化效果：
