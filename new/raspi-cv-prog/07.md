...
此函数接受输入图像，输出图像的深度（-1 表示输入和输出具有相同的深度），以及用于卷积运算的核作为参数。 以下代码演示了此功能的用法：
...
OpenCV `cv2.GaussianBlur()`函数将高斯核应用于输入图像。 我们必须将输入源图像和核大小作为参数传递给此函数的调用。 第三个参数是 X 轴方向上的标准差。 我们为此传递了 0 作为参数。 此功能可以滤除图像中的所有高斯噪声。 以下是此代码示例：
