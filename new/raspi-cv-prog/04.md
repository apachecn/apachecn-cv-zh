...
就像`fswebcam`实用程序一样，`raspistill`实用程序也可以用于捕获时间间隔序列。 在我们先前准备的`timelapse.sh`Shell 脚本中，用适当的`raspistill`命令替换调用`fswebcam`实用程序的行，以记录定时拍摄的照片序列。 然后，使用 RPi 或任何其他 Linux 计算机上的`mencoder`实用程序来创建一段精美的延时视频。
