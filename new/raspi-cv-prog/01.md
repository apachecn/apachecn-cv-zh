...
8.  现在，仅当您使用 VGA 监视器而不是 HDMI 监视器时，才需要执行此步骤。 使用 HDMI 监视器的读者可以放心地忽略此步骤。 可以使用 **Windows File Explorer** 访问 microSD 卡的 BOOT 分区。 它具有`config.txt`文件。 双击并打开文件。 我们必须如下编辑`/boot/config.txt`文件中的设置，以在 VGA 监视器上正确显示：
