...
*   `Imgproc.Scharr(Mat src, Mat dst, int ddepth, int dx, int dy, double scale, double delta)`：这是一个 Scharr 寻边过滤器。 与拉普拉斯滤镜不同，Scharr 滤镜仅查找沿特定方向延伸的边缘，即垂直（如果`dx`为 1 且`dy`为 0）或水平（如果`dx`为 0 且`dy`为 1）的边缘。 对于垂直边缘检测，内核如下：
