...
*   `jni/Application.mk`：此 Makefile 描述将使用 C++ 库的环境类型。 环境的功能包括硬件架构，Android 版本以及应用程序对 C++ 语言功能和标准库的使用。
...
*   《OpenCV 蓝图》（即将出版），由多位作者撰写：这是高级教程和文章的集合，旨在消除有关使用 OpenCV 构建鲁棒应用程序的谜团。 涵盖了理论，实现和基准测试的组合。
