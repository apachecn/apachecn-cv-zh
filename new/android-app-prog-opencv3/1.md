...
*   **OpenCV4Android 3.0 或更高版本**：这是OpenCV的Android版本，包括Java和 C++ 库。
...
7.  编辑系统的`Path`（在Windows中）或`PATH`（在Mac，Linux或其他类似Unix的系统中）以包含`<android_sdk>/platform-tools`，`<android_sdk>/tools`和`<android_ndk>`。 另外，创建一个名为`NDKROOT`的环境变量，并将其值设置为`<android_ndk>`。 （如果不确定如何编辑`Path`，`PATH`或其他环境变量，请参阅本页和下一页框中的提示。）
